# Deployment — first-time setup on a VPS

This guide sets up crusty-proxy on a self-managed host with a dedicated system user, locked-down file permissions, and an internal Docker network shared with the MCP client (e.g. an AI agent such as OpenClaw, or any other MCP-speaking workload).
